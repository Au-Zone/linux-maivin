...
static int vc_mipi_i2c_probe(struct i2c_client *i2c)
{
	...
	csenable =
		devm_gpiod_get_optional(&i2c->dev, "csenable", GPIOD_OUT_HIGH);
	if (IS_ERR(csenable)) {
		retval = PTR_ERR(csenable);
		dev_err(&i2c->dev, "%s(): CANT FIND GPIO!\n", __func__);
	} else {
		dev_err(&i2c->dev, "%s(): CSEnable pin OK!\n", __func__);
	}

	if (csenable) {
		dev_err(&i2c->dev, "%s(): CSEnable high!\n", __func__);
		gpiod_set_value(csenable, 1);
	} else {
		dev_err(&i2c->dev, "%s(): ERROR CSEnable!\n", __func__);
	}
